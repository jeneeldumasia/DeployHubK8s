...
resource "aws_eks_cluster" "main" {
  ...
  vpc_config {
    ...
    endpoint_public_access  = true   # needed for kubectl from CI/CD
  }
...
resource "aws_eks_node_group" "main" {
  ...
  node_group_name = "${var.project}-ng-${var.availability_zones[count.index]}"
  ...
  update_config {
    max_unavailable = 1   # rolling update — keep at least 1 node up per AZ
  }

  labels = {
    "role"                    = "worker"
    "availability-zone"       = var.availability_zones[count.index]
  }
  ...
  tags = merge(var.tags, {
    Name = "${var.project}-node-${var.availability_zones[count.index]}"
  })
}
...
# ── IRSA — Backend Service Account (ECR push + K8s API) ──────────────────────
...
# ── IRSA — EBS CSI Driver (Required for EKS Persistent Volumes) ───────────────
